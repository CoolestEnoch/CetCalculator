...
fun 来点名人名言(): String{
	var xx = 名人名言[名人名言.indices.random()]
	xx = xx.replace("曾经说过", 前面垫话[前面垫话.indices.random()])
	xx = xx.replace("这不禁令我深思", 后面垫话[后面垫话.indices.random()])
	return xx
}

fun 另起一段(): String = with(StringBuilder()) {
//	append("。")
	append("\r\n")
	append("    ")
	toString()
...
fun bullshitEntry(topic: String, length: Int): String = with(StringBuilder()) {
	for (x in topic) {
		while (this.length < length) {
			val branch = (0..100).random()
			when {
				branch < 5 -> {
					append(另起一段())
				}
				branch < 20 -> {
					...
					append(text[text.indices.random()])
				}
			}
		}
	}
	toString().replace("x", topic)
}